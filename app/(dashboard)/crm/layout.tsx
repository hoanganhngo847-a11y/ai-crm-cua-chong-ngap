...
export default async function CrmLayout({
  ...
  // Server-side authorization check enforcing SALE or BOSS_ADMIN
  try {
    ...
    return (
      <div className="p-8 text-center bg-red-950/40 border border-red-800 rounded-xl">
        <h2 className="text-xl font-bold text-red-400 mb-2">Quyền truy cập Kinh doanh (CRM)</h2>
        <p className="text-slate-300 text-sm">
          {error.message || 'Khu vực này chỉ dành cho Nhân viên Kinh doanh hoặc Quản trị viên.'}
        </p>
      </div>
    );
  }
